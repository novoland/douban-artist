...
.factory('Modal', ['$rootScope', '$document', '$compile', '$animate', '$q', 'TemplateLoader', function($rootScope, $document, $compile, $animate, $q, TemplateLoader) {
  var ModalView = ionic.views.Modal.inherit({
    initialize: function(opts) {
      ...
      this.animation = opts.animation || 'modal-show';
    },
    // Show the modal
    show: function() {
      var _this = this,
    ...
    compile: function(element, attr, transclude) {
      return function($scope, $element, $attr) {
        ...
        if($scope.scroll === "false") {
          ...
          var refresherHeight = refresher && refresher.clientHeight || 60;
          ...
          $timeout(function() {
            ...
            if(refresher) {
              sv.activatePullToRefresh(refresherHeight, function() {
                  ...
                  refresher.classList.add('active');  // to refresh
              }, function() {
                refresher.classList.remove('refreshing');  // default
  ...
  return {
    ...
    template: '<header class="bar bar-header nav-bar" ng-class="{invisible: !navController.navBar.isVisible}">' + 
        '<div class="buttons"> ' +
          ...
          '<span ng-click="button.tap($event)" ng-repeat="button in leftButtons" class="button no-animation {{button.type}}" ng-bind-html="button.content"></span>' +
        '</div>' +
...
.directive('sideMenus', function() {
  return {
    ...
    controller: function($scope,$element,$attrs) {
      var _this = this;
      $scope.leftMenuWidth = $attrs.leftMenuWidth;
      $scope.rightMenuWidth = $attrs.rightMenuWidth;

        angular.extend(this, ionic.controllers.SideMenuController.prototype);

      ionic.controllers.SideMenuController.call(this, {
        left: {
          width: $attrs.leftMenuWidth?$attrs.leftMenuWidth:275
        },

        right: {
          width: $attrs.rightMenuWidth?$attrs.rightMenuWidth:275
        }
    ...
    compile: function(element, attr, transclude) {
      return function($scope, $element, $attr, sideMenuCtrl) {
        ...
        /*var dragReleaseFn = function(e) {
          ...
          var dragReleaseFn = function(e) {
              // 为什么要timeout？
              // 在_endDrag中，首先会查看当前menu已经打开的程度，这是根据content的translateX计算而来的，
              // 在某些浏览器中来不及反应，得到的是0，实际上已经打开了一段距离。用$timeout可以解决这个问题。
              $timeout(function(){
                  isDragging = false;
                  if(!defaultPrevented) {
                      sideMenuCtrl._endDrag(e);
        ...
        sideMenuCtrl.setContent({
          ...
          setTranslateX: function(amount) {
            $element[0].style.webkitTransform = 'translateX(' + amount + 'px)';
            $timeout(function() {
    ...
    compile: function(element, attr, transclude) {
      return function($scope, $element, $attr, sideMenuCtrl) {
        ...
        if($scope.side == 'left') {
          ...
          if(sideMenuCtrl.left.width)
            $element[0].style.width = sideMenuCtrl.left.width + 'px';
          ...
          sideMenuCtrl.right.isEnabled = true;
            if(sideMenuCtrl.right.width)
                $element[0].style.width = sideMenuCtrl.right.width + 'px';
...
.directive('tabs', function() {
  return {
    ...
    controller: ['$scope', '$element', '$animate', function($scope, $element, $animate) {
      ...
      this.add = function(controller) {  // 你以为是controller，其实是子tab的scope
